...
class ApiUser : AppCompatActivity(){
    ...
    private fun fetchUserData(userSearch:String) {
        ...
        val request = Request.Builder().url("https://api.github.com/users/$userSearch").build()
    ...
    private fun fetchUserReposData(userSearch: String) {
        val client = OkHttpClient()
        val request = Request.Builder().url("https://api.github.com/users/$userSearch/repos").build()
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e("error", "Exception $e")
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                if(response.isSuccessful){
                    ...
                    runOnUiThread {
                        recycleViewRepo.adapter = MainAdapter(userRepoGitData, this@ApiUser)
                    }
                }
            }
